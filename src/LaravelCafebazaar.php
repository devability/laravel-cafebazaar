...
class LaravelCafebazaar {
            ...
            try {
                $response = $this->guzzle->post("auth/token/", [
                    'form_params' => [
                        ...
                        "redirect_uri" => url(config('laravel-cafebazaar.redirect_uri')),
                   ],
                ]);
    ...
    public function verifyPurchase($package_id, $product_id, $purchase_token,$is_subscription=false) {
        $this->updateToken();
        try {
            if($is_subscription){
                $response = $this->guzzle->get("api/applications/$package_id/subscriptions/$product_id/purchases/$purchase_token?access_token=".$this->data->access_token);
                $data = json_decode($response->getBody()->getContents());
                return new CafebazaarPurchase($data,$is_subscription);
            }else{
            ...
            return new CafebazaarPurchase(new \stdClass(),$is_subscription);
        }
    }
